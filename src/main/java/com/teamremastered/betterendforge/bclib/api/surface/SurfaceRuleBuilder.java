package com.teamremastered.betterendforge.bclib.api.surface;
...
import com.teamremastered.betterendforge.bclib.api.surface.rules.NoiseCondition;
import com.teamremastered.betterendforge.bclib.world.surface.DoubleBlockSurfaceNoiseCondition;
...
public class SurfaceRuleBuilder {
	 ...
	 * @return same {@link SurfaceRuleBuilder} instance.
	 */
	public SurfaceRuleBuilder rule(int priority, RuleSource rule) {
	...
	public RuleSource build() {
		...
		List<RuleSource> ruleList = rules.stream().map(entry -> entry.getRule()).toList();
		RuleSource[] ruleArray = ruleList.toArray(new RuleSource[ruleList.size()]);
