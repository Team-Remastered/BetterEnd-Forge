...
public class VineFeature extends InvertedScatterFeature {
	...
	private BlockState getTopState() {
		...
		return vine ? state.setValue(BlockProperties.TRIPLE_SHAPE, BlockProperties.TripleShape.TOP) : state;
	}
	
	private BlockState getMiggleState() {
		BlockState state = vineBlock.defaultBlockState();
		return vine ? state.setValue(BlockProperties.TRIPLE_SHAPE, BlockProperties.TripleShape.MIDDLE) : state;
	}
	
	private BlockState getBottomState() {
		BlockState state = vineBlock.defaultBlockState();
		return vine ? state.setValue(BlockProperties.TRIPLE_SHAPE, BlockProperties.TripleShape.BOTTOM) : state;
	}
}
