...
public class EndLotusSeedBlock extends UnderwaterPlantWithAgeBlock {
	...
	public void grow(WorldGenLevel world, Random random, BlockPos pos) {
		if (canGrow(world, pos)) {
			...
			BlockState roots = EndBlocks.END_LOTUS_STEM.get().defaultBlockState()
													   .setValue(EndLotusStemBlock.SHAPE, BlockProperties.TripleShape.BOTTOM)
			...
			for (int i = 1; i <= height; i++) {
				if (!world.isEmptyBlock(bpos)) {
					...
					BlocksHelper.setWithoutUpdate(world, bpos, stem.setValue(EndLotusStemBlock.SHAPE, BlockProperties.TripleShape.TOP));
					return;
				}
	...
	private void generateLeaf(WorldGenLevel world, BlockPos pos) {
		...
		BlocksHelper.setWithoutUpdate(world, pos, leaf.setValue(EndLotusLeafBlock.SHAPE, BlockProperties.TripleShape.BOTTOM));
		for (Direction move : BlocksHelper.HORIZONTAL) {
			BlocksHelper.setWithoutUpdate(
				world,
				p.set(pos).move(move),
				leaf.setValue(EndLotusLeafBlock.HORIZONTAL_FACING, move)
					.setValue(EndLotusLeafBlock.SHAPE, BlockProperties.TripleShape.MIDDLE)
			);
		}
		for (int i = 0; i < 4; i++) {
			...
			BlocksHelper.setWithoutUpdate(
				...
				leaf.setValue(EndLotusLeafBlock.HORIZONTAL_FACING, d1)
					.setValue(EndLotusLeafBlock.SHAPE, BlockProperties.TripleShape.TOP)
			);
		}
	}
